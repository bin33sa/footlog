	...
	<div class="container mt-5 mb-5" style="max-width: 900px;">
		<div class="modern-card mb-4 shadow-sm overflow-hidden">
			<div class="article-header p-4 p-md-5 pb-0">
				...
				<div
					class="d-flex justify-content-between align-items-center pb-4 border-bottom">
					...
					<div class="text-muted small">
						<span class="bg-light px-3 py-1 rounded-pill">조회
							${dto.view_count}</span>
					</div>
				</div>
			</div>
			...
			<div class="p-4 p-md-5 pt-0">
				<div class="content-body mt-4">
					...
					<div class="article-text mb-4">${dto.content}</div>

					<%-- article.jsp의 유튜브 출력 부분 --%>
<c:if test="${not empty dto.video_url}">
    <div class="mt-5 ratio ratio-16x9 shadow rounded-4 overflow-hidden border border-5 border-white">
        <%-- 이미 컨트롤러에서 변환되었으므로 그냥 출력만 하면 됩니다 --%>
        <iframe src="${dto.video_url}" 
				...
				<div class="d-flex justify-content-between pt-5 mt-5 border-top">
					...
					<c:if test="${sessionScope.member.member_code == dto.member_code}">
						<div class="d-flex gap-2">
							<a
								href="${pageContext.request.contextPath}/bbs/update?board_main_code=${dto.board_main_code}&page=${page}&category=${category}"
		...
		<div
			class="modern-card p-4 p-md-5 bg-white border-0 shadow-sm rounded-4">
			...
			<div class="bg-light p-3 rounded-4 mb-5">
				<div class="d-flex gap-3">
					...
					<div class="d-flex align-items-end">
						<button
							class="btn btn-point rounded-4 px-4 py-3 fw-bold shadow-sm"
			...
			if (count > 0 && data.listReply) {
				data.listReply
						.forEach(function(item) {
							...
							let photo = item.profile_image ? item.profile_image
									: 'default.png';
							let photoPath = "${pageContext.request.contextPath}/uploads/member/"
									+ photo;
							...
							out += '      <h6 class="fw-bold mb-0 text-dark">'
									+ name + '</h6>';
							out += '      <small class="text-muted" style="font-size:0.75rem;">'
									+ date + '</small>';
							out += '    </div>';
							out += '    <p class="mb-0 text-secondary" style="white-space:pre-wrap; font-size:0.95rem;">'
									+ content + '</p>';
							out += '  </div>';
							out += '</div>';
						});
